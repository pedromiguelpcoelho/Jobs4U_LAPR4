...
/**
 * This class provides a user interface for uploading requirements files.
...
public class UploadRequirementsFileUI extends AbstractUI {
    ...
    protected boolean doShow() {
        ...
        List<JobOpening> jobOpeningList = controller.getAllScreeningJobOpenings();
        if (jobOpeningList.isEmpty()) {
            System.out.println("There are no job openings in Screening Phase.");
            return false;
        }
        ...
        JobOpening selectedJobOpening = controller.getScreeningJobOpeningByIndex(jobOpeningIndex);
        ...
        if (jobApplicationsList.isEmpty()) {
            System.out.println("There are no job applications for this job opening.");
            return false;
        }
        ...
        if (!controller.verifyRequirementsSyntax(filePath)) {
            System.out.println("The file is not syntactically correct. Please try again.");
        } else {
            // Call the controller to upload the file
            boolean success = controller.uploadRequirementsFile(filePath, selectedJobOpening, selectedJobApplication);
    ...
    public String headline() {
        return "Upload Candidate's Requirements";
    }
}